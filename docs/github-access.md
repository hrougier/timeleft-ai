...
- [x] **Rename the board**: *Team Plugins HQ* → **Product Roadmap Board v2** — done
      2026-08-12, along with an intro rewritten for a PO rather than a builder. Page IDs
      survive a rename, so no binding moved with it. The **move** into the product team's
      space is a UI act: the connector can read that parent page but not write to it, so
      `move_pages` returns 404 there and a human drags it.
