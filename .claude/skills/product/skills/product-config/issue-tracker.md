...
Every address below points at the **team's own board** — the one the product team reads.
It began as a private test rig and is not one any more: the mock PRDs are gone, the rows
that remain are real work, and the board is moving into the product team's space. Treat
every page under it as live: no throwaway edits, no test rows, nothing you would not want
a colleague to open. Humans work here alongside the plugin — a page may have moved since
you read it (the `Rev` rule below is how you notice).
